"""
Management command to evaluate swing trading signal performance at different horizons.
Run this daily to track how swing picks performed.

Usage:
    python manage.py evaluate_swing_signal_performance --horizon 2d
    python manage.py evaluate_swing_signal_performance --horizon 3d
    python manage.py evaluate_swing_signal_performance --horizon 5d
    python manage.py evaluate_swing_signal_performance --horizon 7d
    python manage.py evaluate_swing_signal_performance --all
"""
...
from core.signal_performance_models import SwingTradingSignal, SignalPerformance

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    help = 'Evaluate performance of swing trading signals at specified horizons'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            choices=['2d', '3d', '5d', '7d'],
            help='Time horizon to evaluate (2d, 3d, 5d, 7d)'
        )
        parser.add_argument(
            '--all',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if evaluate_all:
            horizons_to_evaluate = ['2d', '3d', '5d', '7d']
        else:
    ...
    def evaluate_horizon(self, horizon, dry_run=False):
        """Evaluate swing signals for a specific time horizon"""
        now = timezone.now()
        
        # Calculate time window based on horizon
        if horizon == '2d':
            time_delta = timedelta(days=2)
            min_age = timedelta(days=1, hours=20)  # Signals must be at least ~2 days old
            max_age = timedelta(days=3)
        elif horizon == '3d':
            time_delta = timedelta(days=3)
            min_age = timedelta(days=2, hours=20)
            max_age = timedelta(days=4)
        elif horizon == '5d':
            time_delta = timedelta(days=5)
            min_age = timedelta(days=4, hours=20)
            max_age = timedelta(days=6)
        elif horizon == '7d':
            time_delta = timedelta(days=7)
            min_age = timedelta(days=6, hours=20)
        ...
        for signal in signals:
            try:
                ...
                # Check time stop (hold days)
                if age.days >= signal.hold_days:
                    hit_time_stop = True
                ...
                # Calculate max favorable/adverse excursion (simplified - would need intraday data for exact)
                # For now, use current price as approximation
                max_favorable = pnl_pct if pnl_pct > 0 else Decimal('0')
                max_adverse = abs(pnl_pct) if pnl_pct < 0 else Decimal('0')
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        f"  {signal.symbol} {signal.side} ({signal.strategy}): {pnl_pct:.2f}% ({outcome})"
                    )
                    ...
                    async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=2.0)) as response:
                        if response.status == 200:
                            ...
                            if results and len(results) > 0:
                                return Decimal(str(results[0].get('c', 0)))
            except:
                pass
