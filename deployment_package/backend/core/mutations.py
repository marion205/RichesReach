...
if not user.is_authenticated:
raise GraphQLError(
    "You must be logged in to remove stocks from your watchlist.")
try:
    # Find the stock
try:
...
return SavePortfolio(
    success=True,
    message=f"Portfolio saved successfully with {
        len(saved_portfolios)} holdings.",
...
content_without_media = content.replace(
    '[IMAGE:', '').replace(
        '[VIDEO:', '').strip()
...
if is_limited:
raise GraphQLError(
    f"Too many password reset attempts. Please try again after {reset_time}.")
try:
...
if is_limited:
raise GraphQLError(
    f"Too many password change attempts. Please try again after {reset_time}.")
