...
class MLRecommendationType(graphene.ObjectType):
    """
    ML-based stock recommendation with explainability.

    This type represents a personalized stock recommendation generated by
    the ML recommendation engine, which combines:
    - User persona classification
    - Collaborative filtering (similar users)
    - Content-based filtering (stock fundamentals)
    """
    symbol = graphene.String(required=True, description="Stock ticker symbol")
    company_name = graphene.String(description="Company name")
    companyName = graphene.String(description="Company name (camelCase)")
    ml_score = graphene.Float(required=True, description="ML recommendation score (0-100)")
    mlScore = graphene.Float(description="ML recommendation score (camelCase)")
    confidence = graphene.Float(description="Confidence in the recommendation (0-1)")
    recommendation = graphene.String(description="BUY, HOLD, or AVOID")
    reasons = graphene.List(graphene.String, description="Human-readable reasons for the recommendation")
    persona_match = graphene.Float(description="How well this stock matches user's investment persona (0-100)")
    personaMatch = graphene.Float(description="Persona match score (camelCase)")
    similar_user_score = graphene.Float(description="Score from collaborative filtering (0-100)")
    similarUserScore = graphene.Float(description="Similar user score (camelCase)")
...
class StockType(DjangoObjectType):
    ...
    "volatility",
    "current_price")
        # Note: beginner_friendly_score removed from fields - use beginnerFriendlyScore resolver instead
        # This ensures the dynamically calculated personalized score is used, not the static DB value
# Add camelCase fields for frontend compatibility
    companyName = graphene.String()  # camelCase field
    currentPrice = graphene.Float()  # camelCase field
    beginnerFriendlyScore = graphene.Int(description="Dynamically calculated beginner-friendly score (0-100)")  # camelCase, uses dynamic resolver
    ...
    def resolve_beginnerFriendlyScore(self, info):
        """Resolve camelCase beginnerFriendlyScore - uses dynamic calculation."""
        return self.resolve_beginner_friendly_score(info)
    ...
    def resolve_beginner_friendly_score(self, info):
        ...
        """
        # Always recalculate based on fundamentals and user profile
        # Don't rely on old stored values which were calculated differently
        ...
        try:
            ...
            if user and not user.is_anonymous:
                ...
                try:
                    ...
                    from core.models import Portfolio, IncomeProfile
                    ...
                    # Get user's income profile for risk tolerance
                    # Note: Using IncomeProfile (not UserProfile which doesn't exist)
                    try:
                        profile = IncomeProfile.objects.get(user=user)
                        risk_tolerance = getattr(profile, 'risk_tolerance', 'Moderate').lower()
                        income_bracket = getattr(profile, 'income_bracket', 'Unknown').lower()

                        logger.debug(f"User {user.id} profile: risk={risk_tolerance}, income={income_bracket}")
                        ...
                        if 'under' in income_bracket or '<50k' in income_bracket:
                            user_adjustment += 8
                        ...
                        # No income profile yet - use moderate defaults
                        logger.debug(f"User {user.id} has no IncomeProfile, using defaults")
                        user_adjustment += 5  # Default moderate adjustment

                    logger.debug(f"User {user.id} adjustment: {user_adjustment} (portfolio: ${portfolio_value:,.0f})")

                except Exception as e:
                    logger.warning(f"Error calculating user adjustment for beginner score: {e}")
