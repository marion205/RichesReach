...
export default function TutorQuizScreen() {
  ...
  const loadQuiz = async () => {
    ...
    try {
      ...
      if (!res) {
        logger.error('❌ [Quiz] No response received');
      ...
      if (!res.questions || res.questions.length === 0) {
        logger.warn('⚠️ [Quiz] No questions in response, using fallback');
        const mockQuiz = {
          topic: topic,
          difficulty: 'beginner',
          questions: [
      ...
      if (invalidQuestions.length > 0) {
        logger.warn('⚠️ [Quiz] Some questions missing options:', invalidQuestions.length);
        // Filter out invalid questions
        res.questions = res.questions.filter(q => q.options && q.options.length > 0);
        if (res.questions.length === 0) {
          logger.error('❌ [Quiz] All questions invalid, using fallback');
      ...
      logger.error('❌ [Quiz] Error loading quiz:', error);
      Alert.alert(
        'Error Loading Quiz',
