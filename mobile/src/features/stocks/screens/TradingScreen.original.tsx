...
import {
  ...
  ActivityIndicator, RefreshControl, SafeAreaView, Dimensions, FlatList, Linking,
} from 'react-native';
...
const TradingScreen = ({ navigateTo }: { navigateTo: (screen: string) => void }) => {
  ...
  // Quote fetch - immediate for initial load, debounced for typing
  const {
    data: quoteData,
    loading: quoteLoading,
    refetch: refetchQuote,
  } = useQuery(GET_TRADING_QUOTE, { 
    variables: { symbol: upper(symbol) || 'AAPL' }, 
    skip: !symbol, 
    errorPolicy: 'all',
  ...
  useEffect(() => {
    ...
    // On initial load, fetch immediately (no debounce)
    ...
    // For subsequent changes (user typing), debounce
  ...
  useEffect(() => {
    if (showOrderModal) {
      ...
      // Fetch quote immediately when modal opens
      if (symbol) {
        refetchQuote?.({ symbol: upper(symbol) });
      }
    }
  }, [showOrderModal, symbol, refetchQuote]);
  ...
  const proceedWithOrder = async () => {
    ...
    try {
      ...
      // Check if user has Alpaca account, if not show connect modal
      if (!alpacaAccount) {
        alpacaAnalytics.track('connect_initiated', { source: 'order_placement' });
        setIsPlacingOrder(false);
        // Close the order modal first (React Native doesn't support nested modals)
        setShowOrderModal(false);
        // Use setTimeout to ensure order modal closes before showing connect modal
        setTimeout(() => {
          setShowConnectModal(true);
        }, 300); // Small delay to allow order modal to close
        return;
      }
      ...
      <SafeAreaView style={styles.modal}>
        ...
        <ScrollView style={{ paddingHorizontal:20 }} showsVerticalScrollIndicator={false}>
          ...
          {(() => {
            ...
            // Get current market price from quote (for all order types)
            ...
            // Use loading state from query, but only show loading if we don't have cached data
            // If query is taking too long, we'll show an estimate based on a reasonable default
            if (quoteLoading && !quoteData?.tradingQuote) {
              // Only show loading spinner if we truly have no data
              // After 1 second, show a fallback estimate
              isLoading = true;
            }

            if (orderType === 'limit') {
              if (price) {
                // User entered a limit price - use it
                pricePerShare = parseFloat(price) || 0;
                priceSource = 'Limit Price';
              } else if (marketPrice > 0) {
                // No limit price yet, but we have market data - show estimate
                pricePerShare = marketPrice;
                priceSource = orderSide === 'buy' ? 'Ask (Est.)' : 'Bid (Est.)';
                isEstimated = true;
              } else {
                // No price and no market data - show loading
                isLoading = true;
              }
            } else if (orderType === 'stop_loss') {
              if (stopPrice) {
                // User entered a stop price - use it
                pricePerShare = parseFloat(stopPrice) || 0;
                priceSource = 'Stop Price';
              } else if (marketPrice > 0) {
                // No stop price yet, but we have market data - show estimate
                pricePerShare = marketPrice;
                priceSource = orderSide === 'buy' ? 'Ask (Est.)' : 'Bid (Est.)';
                isEstimated = true;
              } else {
              ...
              // Market order - always use current market price
              if (quoteData?.tradingQuote) {
                pricePerShare = marketPrice;
                priceSource = orderSide === 'buy' ? 'Ask' : 'Bid';
              } else {
                // No quote data yet - show loading
                isLoading = true;
              }
            }

            // Show estimate if we have valid data, or show loading
              ...
              <View style={styles.orderTotalBox}>
                <View style={styles.orderTotalHeader}>
                  <Text style={styles.orderTotalLabel}>
                    {isEstimated ? 'Estimated ' : ''}{orderSide === 'buy' ? 'Cost' : 'Proceeds'}
                    {isEstimated && ' (at market)'}
                  </Text>
                  {isLoading ? (
                    <ActivityIndicator size="small" color={C.primary} />
                  ) : (
                    <Text style={styles.orderTotalAmount}>
                      ${(qty * pricePerShare).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </Text>
                  )}
                </View>
                {!isLoading && (
                  <View style={styles.orderTotalBreakdown}>
                    <Text style={styles.orderTotalDetail}>
                      {qty} {qty === 1 ? 'share' : 'shares'} × ${pricePerShare.toFixed(2)} ({priceSource})
                    </Text>
                    {isEstimated && (
                      <Text style={[styles.orderTotalDetail, { marginTop: 4, fontSize: 12 }]}>
                        Enter {orderType === 'limit' ? 'limit' : 'stop'} price above to see exact total
                      </Text>
                    )}
                  </View>
                )}
