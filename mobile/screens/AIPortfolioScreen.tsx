...
useEffect(() => {
if (!showProfileForm && userData?.me?.incomeProfile) {
...
const aiRecommendations = recommendationsData?.aiRecommendations;
...
<Text style={styles.summaryValue}>${aiRecommendations.portfolioAnalysis?.totalValue?.toLocaleString() || 'N/A'}</Text>
</View>
<View style={styles.summaryItem}>
<Text style={styles.summaryLabel}>Holdings</Text>
<Text style={styles.summaryValue}>{aiRecommendations.portfolioAnalysis?.numHoldings || 'N/A'}</Text>
</View>
<View style={styles.summaryItem}>
<Text style={styles.summaryLabel}>Risk Score</Text>
<Text style={styles.summaryValue}>{aiRecommendations.portfolioAnalysis?.riskScore || 'N/A'}</Text>
</View>
</View>
...
<Text style={styles.sectionTitle}>Portfolio Allocation</Text>
        <View style={styles.sectorGrid}>
          <View style={styles.sectorItem}>
            <Text style={styles.sectorLabel}>Technology</Text>
            <Text style={styles.sectorValue}>{aiRecommendations.portfolioAnalysis?.sectorBreakdown?.Technology || 0}%</Text>
          </View>
          <View style={styles.sectorItem}>
            <Text style={styles.sectorLabel}>Healthcare</Text>
            <Text style={styles.sectorValue}>{aiRecommendations.portfolioAnalysis?.sectorBreakdown?.Healthcare || 0}%</Text>
          </View>
          <View style={styles.sectorItem}>
            <Text style={styles.sectorLabel}>Financials</Text>
            <Text style={styles.sectorValue}>{aiRecommendations.portfolioAnalysis?.sectorBreakdown?.Financials || 0}%</Text>
          </View>
        </View>
</View>
...
<Text style={styles.sectionTitle}>Quantitative Stock Picks</Text>
        {(aiRecommendations.buyRecommendations || []).map((stock: any, index: number) => (
...
<Text style={styles.allocationText}>{stock.confidence ? (stock.confidence * 100).toFixed(0) : 0}%</Text>
<Text style={styles.expectedReturn}>{stock.expectedReturn ? (stock.expectedReturn * 100).toFixed(1) + '%' : 'N/A'}</Text>
</View>
</View>
<Text style={styles.reasoning}>{stock.reasoning}</Text>
<View style={styles.stockTags}>
<View style={[styles.tag, { backgroundColor: '#10B981' }]}>
<Text style={styles.tagText}>{stock.recommendation}</Text>
</View>
