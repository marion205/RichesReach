...
print(f"⏱ Login request completed in {response_time:.2f}s")
if response.status_code == 200:
data = response.json()
if "errors" in data:
...
print(" Response time is acceptable (2-5s)")
else:
...
end_time = time.time()
response_time = end_time - start_time
print(f"⏱ Database query completed in {response_time:.2f}s")
if response.status_code == 200:
print(" Database query successful")
if response_time < 1.0:
print(" Database performance is good (< 1s)")
elif response_time < 3.0:
print(" Database performance is acceptable (1-3s)")
else:
