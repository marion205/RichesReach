#!/usr/bin/env python3
"""
RichesReach AI Service - Live Data Server
...
app = FastAPI(
    title="RichesReach Live Data Service",
...
POPULAR_STOCKS = [
    ...
    "WMT", "PG", "KO", "PEP", "JNJ", "UNH", "V", "MA", "HD", "DIS"
]
    ...
    try:
        def _fetch():
            ...
            return {
                ...
                "beginnerFriendlyScore": calculate_beginner_score(info, current_price)
            }
...
def calculate_beginner_score(info: dict, current_price: float) -> int:
    """Calculate a beginner-friendly score based on stock metrics"""
    score = 50  # Base score
    ...
    # P/E ratio bonus (reasonable valuations)
    ...
    # Dividend yield bonus (income stocks)
    dividend_yield = info.get('dividendYield', 0)
    if dividend_yield and 0.02 <= dividend_yield <= 0.06:  # 2-6%
        score += 10
    elif dividend_yield and dividend_yield > 0:
        score += 5
    
    # Volume bonus (liquid stocks)
...
async def search_stocks(search_term: str) -> List[Dict[str, Any]]:
    """Search for stocks matching the search term"""
    if not search_term:
        # Return popular stocks if no search term
        symbols = POPULAR_STOCKS[:10]
    else:
        # Filter popular stocks by search term
        search_lower = search_term.lower()
        symbols = [s for s in POPULAR_STOCKS if search_lower in s.lower()]
        
        # If no matches, try to find similar symbols
        if not symbols:
            symbols = [s for s in POPULAR_STOCKS if search_lower[0] == s.lower()[0]][:5]
    
    # Fetch data for matching symbols
...
async def graphql_endpoint(request_data: dict):
    ...
    try:
        ...
        # Handle GetStocks query with real data
            ...
            stocks_data = await search_stocks(search_term)
        ...
        # Handle GetBeginnerFriendlyStocks query
            ...
            # Return ETFs and stable stocks for beginners
            beginner_symbols = ["SPY", "VTI", "QQQ", "VXUS", "BND"]
            tasks = [fetch_stock_data(symbol) for symbol in beginner_symbols]
            results = await asyncio.gather(*tasks, return_exceptions=True)
        ...
        # Handle rustStockAnalysis query
            ...
            if not stock_data:
                return {
                    "errors": [{"message": f"Could not fetch data for {symbol}"}]
                }
...
async def root():
    return {
        "message": "RichesReach Live Data Service",
        "version": "1.0.0",
        "status": "running",
