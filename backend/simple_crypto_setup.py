#!/usr/bin/env python3
"""
Simple crypto setup - create essential tables and seed data
"""
...
def create_crypto_tables():
    """Create essential crypto tables"""
    
    with connection.cursor() as cursor:
        # Drop tables if they exist
        cursor.execute("DROP TABLE IF EXISTS crypto_currencies CASCADE;")
        cursor.execute("DROP TABLE IF EXISTS crypto_prices CASCADE;")
        cursor.execute("DROP TABLE IF EXISTS crypto_portfolios CASCADE;")
        cursor.execute("DROP TABLE IF EXISTS crypto_holdings CASCADE;")
        cursor.execute("DROP TABLE IF EXISTS crypto_trades CASCADE;")
        cursor.execute("DROP TABLE IF EXISTS crypto_ml_predictions CASCADE;")
        cursor.execute("DROP TABLE IF EXISTS crypto_sbloc_loans CASCADE;")
        cursor.execute("DROP TABLE IF EXISTS crypto_education_progress CASCADE;")
        ...
        cursor.execute("""
            CREATE TABLE crypto_holdings (
                id SERIAL PRIMARY KEY,
                portfolio_id INTEGER REFERENCES crypto_portfolios(id) ON DELETE CASCADE,
                cryptocurrency_id INTEGER REFERENCES crypto_currencies(id) ON DELETE CASCADE,
