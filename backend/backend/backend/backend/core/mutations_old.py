...
def _fallback_max_drawdown(risk_tolerance: str) -> float:
    mapping = {
        'Conservative': 15.0,
        'Moderate': 32.0,
        'Aggressive': 45.0,  # give a sensible aggressive placeholder
    }
    return float(mapping.get(risk_tolerance, 32.0))
...
def _compute_expected_impact(stocks, total_value):
    ...
    wsum = 0.0
    for s in stocks:
        er = _normalize_to_decimal(s.get("expectedReturn")) or 0.0  # already decimal after we set it
        conf = s.get("confidence")
        alloc = s.get("allocation")  # percent number like 25.0
        # weight preference: confidence (already 0..1) else allocation%
        if conf is not None:
            try:
                w = float(conf)
                # if someone accidently passes 70 (percent), scale it
                if abs(w) > 1.5:
                    w = w / 100.0
            except (TypeError, ValueError):
                w = 0.5
            ...
            try:
                w = max(0.0, min(1.0, float(alloc or 0.0) / 100.0))
            except (TypeError, ValueError):
                w = 0.0
    ...
    ev_return_decimal = (weighted / wsum) if wsum > 0.0 else None
    ev_change_for_total_value = (ev_return_decimal * total_value) if (ev_return_decimal is not None and total_value) else None
    ev_per_10k = (10000.0 * ev_return_decimal) if ev_return_decimal is not None else None

    return {
        "ev_return_decimal": ev_return_decimal,
        "ev_change_for_total_value": ev_change_for_total_value,
        "ev_per_10k": ev_per_10k,
    }
    ...
    def mutate(self, info):
        ...
        try:
            ...
            expected_return = sum(
                ...
                # Multiple moderate portfolio options
                moderate_options = [
                    # Option 1: Traditional Moderate
                    [
                        {
                            'symbol': 'SPY',
                            ...
                            'riskLevel': 'Medium',
                            'expectedReturn': _normalize_to_decimal(9.2),
                            'confidence': 0.65,
                            'recommendation': 'BUY',
                        },
                        {
                            ...
                            'expectedReturn': _normalize_to_decimal(11.5),
                            'confidence': 0.6,
                            'recommendation': 'BUY',
                        },
                        {
                            ...
                            'riskLevel': 'Medium',
                            'expectedReturn': _normalize_to_decimal(8.1),
                            'confidence': 0.55,
                            'recommendation': 'BUY',
                        },
                        {
                            ...
                            'riskLevel': 'Low',
                            'expectedReturn': _normalize_to_decimal(4.2),
                            'confidence': 0.7,
                            'recommendation': 'BUY',
                        }
                    ],
                    # Option 2: Growth-Focused Moderate
                    [
                        {
                            'symbol': 'VUG',
                            'companyName': 'Vanguard Growth ETF',
                            'allocation': 40.0,
                            'reasoning': 'Growth-oriented large-cap stocks',
                            'riskLevel': 'Medium',
                            'expectedReturn': _normalize_to_decimal(10.5),
                            'confidence': 0.65,
                            'recommendation': 'BUY',
                        },
                        {
                            ...
                            'allocation': 20.0,
                            'reasoning': 'Innovation and disruptive technology',
                            'riskLevel': 'High',
                            'expectedReturn': _normalize_to_decimal(15.0),
                            'confidence': 0.5,
                            'recommendation': 'BUY',
                        },
                        {
                            ...
                            'allocation': 20.0,
                            'reasoning': 'Developed international markets',
                            'riskLevel': 'Medium',
                            'expectedReturn': _normalize_to_decimal(8.5),
                            'confidence': 0.6,
                            'recommendation': 'BUY',
                        },
                        {
                            ...
                            'allocation': 20.0,
                            'reasoning': 'Investment-grade corporate bonds',
                            'riskLevel': 'Low',
                            'expectedReturn': _normalize_to_decimal(5.5),
                            'confidence': 0.7,
                            'recommendation': 'BUY',
                        }
                    ]
                ...
                # Randomly select one of the moderate options
                selected_option = random.choice(moderate_options)
                recommended_stocks = selected_option
                expected_return = _normalize_to_decimal(8.8)  # 0.088
                risk_assessment_text = "Balanced portfolio with growth potential and moderate risk."
                ...
                recommended_stocks = [
                    {
                        ...
                        'riskLevel': 'High',
                        'expectedReturn': _normalize_to_decimal(12.5),
                        'confidence': 0.55,
                        'recommendation': 'BUY',
                    },
                    {
                        ...
                        'riskLevel': 'Very High',
                        'expectedReturn': _normalize_to_decimal(15.2),
                        'confidence': 0.5,
                        'recommendation': 'BUY',
                    },
                    {
                ...
                expected_return = _normalize_to_decimal(11.8)  # 0.118
                risk_assessment_text = "Aggressive growth portfolio with high potential returns and significant volatility."
    ...
    def mutate(self, info, stock_symbol, notes=None):
        ...
        try:
            ...
            watchlist, created = Watchlist.objects.get_or_create(
                ...
                name="My Watchlist",
                defaults={
                    'description': 'My personal stock watchlist',
                    'is_public': False,
                    'is_shared': False
                }
            )
            ...
            watchlist_item, created = WatchlistItem.objects.get_or_create(
                ...
                stock=stock,
                defaults={
                    'notes': notes or f"Added {stock_symbol} to watchlist"
                }
            )
    ...
    def mutate(self, info, stock_symbol):
        ...
        try:
            ...
            try:
                ...
                return RemoveFromWatchlist(
                    success=False,
                    message="You don't have a watchlist."
                )
            
            # Remove from watchlist
            watchlist_item = WatchlistItem.objects.filter(watchlist=watchlist, stock=stock).first()
            if watchlist_item:
...
class Mutation(graphene.ObjectType):
    ...
    # ML-Enhanced Portfolio Recommendations
    ...
    # Institutional-Grade Portfolio Recommendations
    generate_institutional_portfolio_recommendation = GenerateInstitutionalPortfolioRecommendation.Field()
    
    # Monitoring and Health Checks
    get_system_health = MonitoringMutations.get_system_health
    get_system_metrics = MonitoringMutations.get_system_metrics
    get_monitoring_dashboard = MonitoringMutations.get_monitoring_dashboard
    get_audit_logs = MonitoringMutations.get_audit_logs
    get_ml_model_versions = MonitoringMutations.get_ml_model_versions