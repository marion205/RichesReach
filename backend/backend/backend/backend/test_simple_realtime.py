#!/usr/bin/env python3
"""
Test script for simple real-time data integration
"""
import os
import sys
import asyncio
...
async def test_simple_realtime():
    """Test the simple real-time data integration"""
    print("🚀 Testing Simple Real-Time Data Integration...")
    
    # Test 1: Update a single stock
    print("\n📊 Test 1: Updating AAPL with real-time data...")
    try:
        success = await simple_realtime_service.update_stock_in_database('AAPL')
        if success:
    ...
    try:
        results = await simple_realtime_service.update_priority_stocks(priority_symbols)
    ...
    # Test 3: Get price data for a stock
    print("\n📊 Test 3: Getting price data for TSLA...")
    try:
        data = await simple_realtime_service.get_stock_price_data('TSLA')
        if data:
            print(f"   ✅ TSLA data: ${data.get('price', 'N/A')}")
    ...
    print("\n🎉 Simple real-time integration test complete!")
