...
# stock_sentiment = graphene.Field(StockSentimentType, stock_symbol=graphene.String())
    ...
    def resolve_stock_discussions(self, root, info, stock_symbol=None):
        if stock_symbol:
    ...
    def resolve_trending_discussions(self, root, info):
# Get discussions with most likes in the last 7 days
    ...
    def resolve_user_discussions(self, root, info, user_id=None):
        if user_id:
